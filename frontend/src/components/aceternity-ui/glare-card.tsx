...
export const GlareCard = ({
  ...
  return (
    <div
      style={containerStyle}
      className="relative isolate w-full [contain:layout_style] [perspective:600px] transition-transform duration-[var(--duration)] ease-[var(--easing)] delay-[var(--delay)] will-change-transform"
      ref={refElement}
      onPointerMove={(event) => {
        const rotateFactor = 0.4;
        const rect = event.currentTarget.getBoundingClientRect();
    ...
    >
      <div className="h-full grid will-change-transform transition-transform duration-[var(--duration)] ease-[var(--easing)] delay-[var(--delay)] rounded-[var(--radius)] border border-slate-800 overflow-hidden">
